...
public class fouranim extends Base {


    public fouranim(Context c, AsyncResponse asyncResponse)
    {
    ...
    public void onPostExecute(Document document)
    {
        try
        {
            this.videoUrl= document.select("source[type=\"video/mp4\"]").last().attr("src");
            if(! this.videoUrl.equals("") && this.videoUrl !=null )
            {
                String ThisUrl = this.videoUrl;
                StringBuilder sb = new StringBuilder();
                sb.append(mainContext.getResources().getString(R.string.fourAnim_Suffix));
                sb.append(System.currentTimeMillis());
                sb.append(".mp4");
                String str3 = Utils.RootDirectoryfouranime;
