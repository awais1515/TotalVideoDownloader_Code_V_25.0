...
public class Funimate  extends Base {

    public Funimate(Context c, AsyncResponse asyncResponse) {
    ...
    public void onPostExecute(Document document)
    {
        try
        {
            this.videoUrl= document.select("meta[property=\"twitter:player:stream\"]").last().attr("content");

            if(! this.videoUrl.equals("") && this.videoUrl !=null )
            {
                String ThisUrl = this.videoUrl;
                StringBuilder sb = new StringBuilder();
                sb.append(mainContext.getResources().getString(R.string.Funimate_Suffix));
                sb.append(System.currentTimeMillis());
                sb.append(".mp4");
                String str3 = Utils.RootDirectoryFunimate;
