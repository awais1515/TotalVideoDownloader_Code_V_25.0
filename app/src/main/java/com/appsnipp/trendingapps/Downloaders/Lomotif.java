...
public class Lomotif extends Base {
    ...
    public Lomotif(Context c, AsyncResponse asyncResponse)
    {
    ...
    public void onPostExecute(Document document)
    {
        try
        {
            ...
            if (!html.equals("")) {
                this.videoUrl = String.valueOf(new JSONObject(html).getJSONObject("props").getJSONObject("pageProps").getJSONObject("lomotif").get("video"));
                if(! this.videoUrl.equals("") && this.videoUrl !=null )
                {
                    String ThisUrl = this.videoUrl;
                    StringBuilder sb = new StringBuilder();
                    sb.append(mainContext.getResources().getString(R.string.Lomotif_Suffix));
                    sb.append(System.currentTimeMillis());
                    sb.append(".mp4");
                    String str3 = Utils.RootDirectoryLomotif;
